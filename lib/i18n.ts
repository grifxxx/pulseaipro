...
const STRINGS: Record<Locale, UIStrings> = {
  ru: {
    ...
    navFaq: "Вопросы",
    navBlog: "Статьи",
    homeHeroTitle: "Статьи для частного инвестора",
    homeHeroSubtitle:
    ...
    homeCtaGuides: "Читать статьи",
    homeCtaFeed: "Смотреть ленту",
    homeStatGuides: (n) => `${n} ${n % 10 === 1 && n % 100 !== 11 ? "статья" : n % 10 >= 2 && n % 10 <= 4 && (n % 100 < 10 || n % 100 >= 20) ? "статьи" : "статей"}`,
    ...
    homeUpcomingHint: "Самые частые вопросы по теме — над ними сейчас работаем. Новые статьи выходят по понедельникам, средам и пятницам.",
    ...
    homeTelegramText: "Новые статьи и заметные движения рынка — по мере выхода, без ежедневного шума.",
    ...
    archiveSubtitle:
      "Автоматические обзоры рынка, которые выходили до перехода на статьи. Сохранены как есть и больше не обновляются.",
    archiveBackToGuides: "← К статьям",
    ...
    aboutAuthorBody: [
      ...
      "Статьи готовит ИИ-система, которую он же и построил: тема, план и требования к каждому материалу задаются заранее — прямой ответ на вопрос в первом абзаце, расчёт с числами, никаких призывов покупать или продавать. Что выходит на сайте и под чьим именем, решает автор, и он же за это отвечает.",
    ...
    blogEmptyState: "Статей пока нет — первые появятся в ближайшие дни.",
