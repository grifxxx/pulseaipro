...
const PERIOD_DOT = "bg-muted";
const HUMOR_DOT = "bg-negative";
  ...
  return (
    <article
      className={`group flex flex-col overflow-hidden rounded-2xl border transition-all hover:shadow-lg hover:shadow-black/[0.03] dark:hover:shadow-black/20 ${
        isSponsored
          ? "border-warning/30 bg-warning-soft hover:border-warning/60"
    ...
    >
      ...
      <div className="flex flex-col gap-2 p-5">
        ...
        <Link href={`/blog/${article.slug}`} className="font-serif text-lg font-semibold tracking-tight leading-snug group-hover:text-accent transition-colors">
          {article.title}
        </Link>
